...
// Apaleo rate plan code → IBE display mapping for MUBRIG (Hotel Mulin)
...
function getRatePlanMapping(offer) {
  ...
  // Tolerate property-prefixed codes ("MUBRIG-NONREFBB_IBE") or extra suffixes
...
var UNIT_GROUP_NAME_MAP = [
  { match: ['double room with balcony', 'doppelzimmer mit balkon', 'double balcony'], nameKey: 'rooms.name_dbrb', fallback: 'Double Room with Balcony' },
  { match: ['double room normal', 'double room', 'doppelzimmer'], nameKey: 'rooms.name_dbln', fallback: 'Double Room' },
  { match: ['galery suite', 'gallery suite', 'galerie suite', 'galerie zimmer', 'gallery room'], nameKey: 'rooms.name_glrs', fallback: 'Gallery Suite' },
  { match: ['family room 6', 'familienzimmer 6', 'familienzimmer (6)', 'family room'], nameKey: 'rooms.name_fam6', fallback: 'Family Room (6)' },
  { match: ['single room big', 'einzelzimmer gross'], nameKey: 'rooms.name_sglb', fallback: 'Single Room Big' },
  { match: ['single room normal', 'single room', 'einzelzimmer'], nameKey: 'rooms.name_sgln', fallback: 'Single Room' }
];
...
function getOfferCityTax(offer) {
  // City tax already included in the rate (MUBRIG): never add it on top.
  if (!offer || !offer.cityTax || !offer.cityTax.amount || offer.cityTax.included) return 0;
...
function fetchOffers() {
  ...
  .catch(function (err) {
    ...
    helpP.textContent = window.t ? window.t('booking.error_help') : 'Bitte versuchen Sie es erneut oder kontaktieren Sie uns unter mulin@amanthos.com';
...
function renderOfferCard(offer, categoryClass, index, isBestPrice) {
  ...
  } else if (offer.cityTax && offer.cityTax.included && offer.cityTax.amount > 0) {
    html += '<div class="offer-citytax" style="font-size:.74rem;color:var(--color-text-muted);margin-top:.2rem;">' + tFallback('booking.citytax_included', 'inkl. Kurtaxe') + '</div>';
  }
  html += '</div>';
...
function renderUpsells() {
  ...
  UPSELL_ITEMS.forEach(function (item) {
    ...
    var priceLabel = 'CHF ' + (priceCents / 100).toFixed(2);
...
function getUpsellTotal() {
  ...
  Object.keys(selectedUpsells).forEach(function (id) {
    ...
    var priceCents = Math.round(item.price * 100);
    totalCents += item.perDay ? priceCents * nights : priceCents;
  });
...
function getUpsellComment() {
  ...
  Object.keys(selectedUpsells).forEach(function (id) {
    ...
    if (item.perDay && nights > 1) {
      ...
      items.push(item.nameFallback + ' (CHF ' + (priceCents / 100).toFixed(2) + ' x ' + nights + ' = CHF ' + (totalCents / 100).toFixed(2) + ')');
    } else {
      items.push(item.nameFallback + ' (CHF ' + (priceCents / 100).toFixed(2) + ')');
    }
  });
...
if (confirmBtn) {
  confirmBtn.addEventListener('click', function () {
    ...
    var commentParts = ['Booked via hotelmulin.ch'];
    ...
    Object.keys(selectedUpsells).forEach(function (id) {
      var item = selectedUpsells[id];
      var priceCents = Math.round(item.price * 100);
      var totalCents = item.perDay ? priceCents * bookingNights : priceCents;
      upsellsList.push({ id: item.id, name: item.nameFallback, price: totalCents / 100, unitPrice: item.price, perDay: !!item.perDay, nights: item.perDay ? bookingNights : 1 });
    });
...
function showPaymentStep(confirmationId, paymentLink, email, bookingData) {
  ...
  html += (window.t ? window.t('booking.not_confirmed_yet') : 'Du hast noch keine g\u00fcltige Buchung!');
  html += '</p>';
  html += '<p style="color:#991B1B;font-size:.85rem;margin:.4rem 0 0;">' + (window.t ? window.t('booking.will_be_cancelled') : 'Ohne Zahlung wird die Buchung in 10 Minuten storniert.') + '</p>';
  html += '</div>';
  ...
  if (payBtn) {
    payBtn.addEventListener('click', function () {
      ...
      // Desktop: open popup
      var popup = window.open(paymentLink, 'hotelmulin_payment', 'width=900,height=700,scrollbars=yes,resizable=yes');
...
function showPaymentRetry(confirmationId, email, bookingData) {
  ...
  html += '<p style="font-size:.8rem;color:var(--color-text-muted);margin-top:1rem;">' + (window.t ? window.t('booking.or_contact') : 'Oder kontaktieren Sie uns:') + ' <a href="mailto:mulin@amanthos.com" style="color:var(--color-accent);">mulin@amanthos.com</a></p>';
  html += '</div>';
